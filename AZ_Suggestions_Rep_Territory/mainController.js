  ...
  //      var inOwner = ds.getInStatement(userIds);
   //     $('#response').append('<div> Owner :' + JSON.stringify(inOwner, null, "\t") + '</div>');
	...
	function filterSuggestions(productFilter, driverFilter) {
		...
		$('#response').append('<pre>selected_products :'+ JSON.stringify(selected_products, null, "\t") +' </pre>');
		$('#response').append('<pre>selected_drivers :'+ JSON.stringify(selected_drivers, null, "\t") +' </pre>');
		try {
			var filtered = {},
				tableDataCount = 0,
				thisSuggestion = {},
